...
import { ShoppingCart, Home, ChevronRight, Heart } from 'lucide-react';
import { usePizzaStore } from '../store/pizzaStore';
import { useCartStore } from '../store/cartStore';
import { useFavoritePizza } from '../components/FavoritePizza';
...
const PizzaDetail = () => {
  ...
  const { addToFavorites } = useFavoritePizza();
  ...
  // Fallback obrázok - lokálny placeholder
  const FALLBACK_IMAGE = '/pizza-placeholder.jpg';
  ...
  return (
    <div className="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex mb-4 gap-2">
        <Link 
          to="/menu"
          className="inline-flex items-center px-5 py-3 bg-[#4a5d23] text-white rounded-lg hover:bg-[#3d4d1c] transition-colors shadow-md font-medium"
        >
          <ChevronRight className="w-5 h-5 rotate-180 mr-2" />
          Späť na ponuku pizze
        </Link>
        <Link 
          to="/"
          className="inline-flex items-center px-4 py-2 bg-gray-100 text-[#4a5d23] border border-[#4a5d23] rounded-lg hover:bg-gray-200 transition-colors"
        >
          ...
          <div className="nature-box bg-white bg-opacity-95 overflow-hidden border-2">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="md:w-2/5 relative">
                <div className="aspect-video md:aspect-auto md:h-full bg-gradient-to-br from-amber-50 to-orange-50 flex items-center justify-center">
                  <PizzaIcon pizzaName={pizza.name} size={200} />
                </div>
              ...
              <div className="p-6 md:w-3/5 md:px-8 flex flex-col">
                ...
                <div className="mt-6 border-t border-[#e0e8c9] pt-4">
                  <div className="flex items-center gap-4">
                    ...
                    <button 
                      onClick={() => {
                        const customName = prompt('Zadajte názov pre obľúbenú pizzu:', `${pizza.name} (moja obľúbená)`);
                        if (customName) {
                          addToFavorites(customName, pizza, selectedSize, selectedExtras);
                        }
                      }}
                      className="px-4 py-3 border-2 border-[#4a5d23] text-[#4a5d23] rounded-lg font-medium hover:bg-[#4a5d23] hover:text-white transition duration-200 flex items-center justify-center"
                      title="Uložiť ako obľúbenú"
                    >
                      ...
                      <div className="flex md:flex-row flex-col h-full">
                        <div className="md:w-1/3 w-full relative">
                          <img 
                            ...
                            loading="lazy"
                            onError={(e) => {
                              // Pri chybe nastavíme fallback obrázok
