          ...
          {extraCategories
            ...
            .map((extra) => (
              <button 
                ...
                className={`ingredient-tag px-3 py-2 text-sm rounded-md border transition-colors ${
                  isExtraSelected(extra) 
                    ? 'bg-[#4a5d23] text-white border-[#4a5d23] font-medium shadow-sm' 
                    : 'bg-white text-[#4a5d23] border-[#c9d6a3] hover:bg-[#f5f9ee] hover:border-[#4a5d23]/60'
                }`}
              ...
              >
                <div className="flex items-center justify-between gap-2 text-left">
                  <div className="flex-1 min-w-0">
                    ...
                    {extra.amount && (
                      <span className={`text-xs ml-1 ${isExtraSelected(extra) ? 'text-white/80' : 'text-[#4a5d23]/70'}`}>
                        ({extra.amount})
                      </span>
                    )}
                  </div>
                  <span className={`text-xs font-bold whitespace-nowrap ${isExtraSelected(extra) ? 'text-white' : 'text-[#4a5d23]'}`}>
          ...
          <div className="mt-4 p-3 bg-[#f5f9ee] rounded-md border border-[#c9d6a3]">
            ...
            <div className="flex flex-wrap gap-1">
              {selectedExtras.map(extra => (
                <div key={extra.id} 
                  className="px-2 py-1 text-xs rounded bg-[#4a5d23] text-white flex items-center gap-1"
                >
                  <span>{extra.name} {extra.amount && <span className="text-white/80">({extra.amount})</span>}</span>
                  <span className="font-bold">+{extra.price.toFixed(2)}€</span>
                  <button 
                    onClick={() => handleExtraToggle(extra)}
                    className="text-white/70 hover:text-white font-bold ml-1"
